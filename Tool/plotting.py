...
def series_variation(dynamic=None, window_sizes=None, name=None, root=None):
    ...
    plt.savefig(root + '/results_images/Syd/' + name + '_window_size.png', bbox_inches='tight')
    plt.show()
...
def ablation(series,win_size, lam, gam,root,name):
    ...
    sns.heatmap((Z_KBDR > 0).astype(np.int_), annot=False, xticklabels=False, yticklabels=False,
                square=True, cmap="YlGnBu_r", cbar=False, cbar_kws={"shrink": .39})
    plt.savefig(root + '/results_images/Syd/' + name + '_Compare_with_SOAT.png', bbox_inches='tight')
    plt.show()
...
def Z_matrix(Z,root,name):
    ...
    plt.xticks(rotation=0)
    plt.savefig(root + '/results_images/Syd/' + name + '_First_2_matrix.png', bbox_inches='tight')
    plt.show()
...
def Multi_scale(series, data, i, name, root):
    ...
    fig.savefig(root + '/results_images/Syd/' + name + '_multi_scale', bbox_inches='tight', dpi=150)
    plt.show()

def concept(split_series_global,Z,root,name):
    print('plot concepts')
...
def predict(a,b,root,name):
    ...
    plt.savefig(root + '/results_images/Syd/' + name + '_predict.png', bbox_inches='tight')
